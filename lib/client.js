...
function TidalAPI(authData) {
  if(typeof authData !== 'object')
  {
    ...
    console.log('Error :: You must pass auth data into the TidalAPI object correctly');
  } else {
    if(typeof authData.username !== 'string') {
      //throw new Error('Username invalid or missing');
      console.log('Error :: Username invalid or missing');
    }
    if(typeof authData.password !== 'string') {
      //throw new Error('Password invalid or missing');
      console.log('Error :: Password invalid or missing');
    }
    ...
    if(typeof authData.quality !== 'string') {
      ...
      console.log('Error :: Stream quality invalid or missing');
    }
  }
...
TidalAPI.prototype.tryLogin = function(authInfo, cb) {
  ...
  request({
    method: 'POST',
    uri: '/login/username?token=hZ9wuySZCmpLLiui',
    headers: {
...
TidalAPI.prototype.search = function (query, callback) {
  ...
  self._baseRequest('/search', {
      ...
      types: query.type || 'tracks,albums,artists,playlists',
...
* @param {{id: Number, limit: Number, filter: String, offset: Number}}
*/
TidalAPI.prototype.getFeaturedAlbums = function (query, callback) {
  var self = this;
  self._baseRequest('/pages/show_more_featured_albums', {
      limit: query.limit || 999,
      filter: query.filter || 'ALL',
      offset: query.offset || 0,
      countryCode: _countryCode,
      locale: 'en_' + _countryCode,
      deviceType: 'BROWSER'
  }, 'albums', callback);
}
/**
 ...
 * @param {{id: Number}}
 */
TidalAPI.prototype.getFavoriteTracks = function (user, callback) {
    var self = this;
    self._baseRequest('/users/' + (user.id || user) + "/favorites/tracks", {
        limit: user.limit || 999,
        offset: user.offset || 0,
        countryCode: _countryCode
    }, 'favoriteTracks', callback);
}
/**
 * Get user favorite albums.
 * @param {{id: Number}}
 */
TidalAPI.prototype.getFavoriteAlbums = function (user, callback) {
    var self = this;
    self._baseRequest('/users/' + (user.id || user) + "/favorites/albums", {
        limit: user.limit || 999,
        offset: user.offset || 0,
        countryCode: _countryCode
    }, 'favoriteAlbums', callback);
}
/**
 * Get user favorite artists.
 * @param {{id: Number}}
 */
TidalAPI.prototype.getFavoriteArtists = function (user, callback) {
    var self = this;
    self._baseRequest('/users/' + (user.id || user) + "/favorites/artists", {
        limit: user.limit || 999,
        offset: user.offset || 0,
        countryCode: _countryCode
    }, 'favoriteArtists', callback);
}
/**
 * Get user favorite artists.
 * @param {{id: Number}}
 */
TidalAPI.prototype.getFavoritePlaylists = function (user, callback) {
    var self = this;
    self._baseRequest('/users/' + (user.id || user) + "/favorites/playlists", {
        limit: user.limit || 999,
        offset: user.offset || 0,
        countryCode: _countryCode
    }, 'favoritePlaylists', callback);
}
/**
* Get track stream URL.
...
TidalAPI.prototype._baseRequest = function(method, params, type, callback) {
  ...
  if (!self.loggedIn) {
    return self.tryLogin(this.authData, function() {
